...
display(Markdown("## 🏢 Tabel Jumlah Karyawan per Unit Tiap Bulan (Tanpa Status Kontrak/Tetap)"))

# 2. Urutkan bulan
urutan_bulan = ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni']
df['Bulan'] = pd.Categorical(df['Bulan'], categories=urutan_bulan, ordered=True)

# 3. Hitung jumlah unik per Bulan dan Unit saja (tidak pakai status)
jumlah_per_bulan_unit = df.groupby(
    ['Bulan', 'NAMA UNIT\n']
)['NO. INDUK\n'].nunique().reset_index()
jumlah_per_bulan_unit.columns = ['Bulan', 'Unit', 'Jumlah']

# 4. Pivot agar Unit jadi index dan Bulan jadi kolom
pivot_table = jumlah_per_bulan_unit.pivot_table(
    index='Unit',
    columns='Bulan',
    values='Jumlah',
    fill_value=0
...
pivot_table_final = pd.concat([pivot_table, total_row])

# 7. Fungsi pewarnaan
def highlight_total(row):
    if row.name == 'TOTAL':
        return ['background-color: black; color: white'] * len(row)
    return ['background-color: [1ABC9C; color: white'] * len(row)

# 8. Tampilkan tabel
styled_table = pivot_table_final.style.apply(highlight_total, axis=1) \
                                      .set_properties(**{'text-align': 'right'}) \
                                      .format("{:,.0f}")
...
# 3. Hitung jumlah unik per Bulan dan Unit (tanpa status)
jumlah_per_bulan_unit = df.groupby(
    ['Bulan', 'NAMA UNIT\n']
)['NO. INDUK\n'].nunique().reset_index()
jumlah_per_bulan_unit.columns = ['Bulan', 'Unit', 'Jumlah']

# 4. Buat pivot: Unit jadi index, Bulan jadi kolom
pivot_table = jumlah_per_bulan_unit.pivot_table(
    index='Unit',
    columns='Bulan',
    values='Jumlah',
...
# Pastikan index memiliki nama "Unit"
pivot_table_final.index.name = 'Unit'

# 7. Fungsi pewarnaan baris
def highlight_total(row):
    if row.name == 'TOTAL':
        return ['background-color: black; color: white'] * len(row)
    return ['background-color: #1ABC9C; color: white'] * len(row)
...
# 9. Buat grafik garis per unit
# Hapus baris TOTAL agar tidak mengganggu grafik
data_grafik = pivot_table_final.drop(index='TOTAL', errors='ignore')

# Hitung total karyawan per unit (tanpa TOTAL)
total_per_unit = pivot_table.drop(index='TOTAL', errors='ignore').sum(axis=1).sort_values()

# Plot horizontal bar chart
plt.figure(figsize=(10, 6))
total_per_unit.plot(kind='barh', color='#1ABC9C')

plt.title("📊 Total Karyawan per Unit (Selama 6 Bulan)")
plt.xlabel("Total Karyawan")
plt.ylabel("Unit")
plt.tight_layout()
plt.grid(axis='x')
plt.show()
...
# 3. Hitung jumlah unik per Bulan dan Pendidikan (tanpa status)
jumlah_per_bulan_pendidikan = df.groupby(['Bulan', 'PENDIDIKAN_NORMAL'])['NO. INDUK\n'].nunique().reset_index()
jumlah_per_bulan_pendidikan.columns = ['Bulan', 'Pendidikan', 'Jumlah']

# 4. Pivot table: pendidikan = index, bulan = kolom
pivot_pendidikan = jumlah_per_bulan_pendidikan.pivot_table(
    index='Pendidikan',
    columns='Bulan',
    values='Jumlah',
    fill_value=0
...
# 5. Urutkan kolom bulan
pivot_pendidikan = pivot_pendidikan[urutan_bulan]

# 6. Tambahkan baris total
total_row = pd.DataFrame(pivot_pendidikan.sum(axis=0)).T
total_row.index = ['TOTAL']
pivot_pendidikan = pd.concat([pivot_pendidikan, total_row])

# 7. Tampilkan tabel
styled_table = pivot_pendidikan.style \
    .set_properties(**{'text-align': 'right'}) \
    .format("{:,.0f}")

display(Markdown("### 🎓 Tabel Jumlah Karyawan per Bulan berdasarkan Jenis Pendidikan"))
display(styled_table)

# 8. Diagram batang horizontal: Total per Pendidikan (tanpa TOTAL)
total_per_pendidikan = pivot_pendidikan.drop(index='TOTAL', errors='ignore').sum(axis=1).sort_values()

# 9. Plot horizontal bar chart
plt.figure(figsize=(10, 6))
total_per_pendidikan.plot(kind='barh', color='#1ABC9C')
plt.title("📊 Total Karyawan Berdasarkan Pendidikan (6 Bulan)", fontsize=14)
...
# 3. Hitung jumlah unik per Bulan dan Jenis Kelamin
jumlah_per_bulan_sex = df.groupby(['Bulan', 'SEX_NORM'])['NO. INDUK\n'].nunique().reset_index()
jumlah_per_bulan_sex.columns = ['Bulan', 'Jenis Kelamin', 'Jumlah']

# 4. Pivot table: index = Jenis Kelamin, kolom = Bulan
pivot_sex = jumlah_per_bulan_sex.pivot_table(
    index='Jenis Kelamin',
    columns='Bulan',
    values='Jumlah',
...
# 9. Buat Pie Chart total jenis kelamin
total_per_sex = pivot_sex.loc[['Laki-laki', 'Perempuan']].sum(axis=1)

plt.figure(figsize=(6, 6))
colors = ['#3498DB', '#E91E63']
plt.pie(total_per_sex, labels=total_per_sex.index, autopct='%1.1f%%', colors=colors, startangle=90)
plt.title("👥 Persentase Karyawan berdasarkan Jenis Kelamin (Total Januari–Juni)")
plt.axis('equal')
plt.tight_layout()
plt.show()
...
def highlight_sex(row):
    if row.name == 'Laki-laki':
        ...
        return ['background-color: #E91E63; color: white'] * len(row)
    elif row.name == 'TOTAL':
        return ['background-color: black; color: white'] * len(row)
    else:
        return [''] * len(row)
...
# 9. Ambil data bulan terakhir (Juni) untuk pie chart
data_juni = pivot_sex['Juni']

# 10. Pie chart untuk bulan Juni
plt.figure(figsize=(6, 6))
colors = ['#3498DB', '#E91E63']
plt.pie(data_juni, labels=data_juni.index, autopct='%1.1f%%', colors=colors, startangle=90)
plt.title("👥 Persentase Karyawan berdasarkan Jenis Kelamin (Juni)")
plt.axis('equal')
plt.tight_layout()
plt.show()
